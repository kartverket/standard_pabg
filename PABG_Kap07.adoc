...
* Størrelsen på kontrollflaten er avhengig av den planlagte punkttettheten. Ved punkttetthet fra 0,5 punkt/m^2^ til 3 punkt/m^2^ skal størrelse på kontrollflaten være minimum 16 m^2^. Ved planlagt punkttettheten på 20 punkt/m^2^ er minimumsstørrelsen 4 m^2^, se <<tab-kontrollflate>>.


* Rundt en kontrollflate skal det være 0,5 m buffersone fri for obstruksjoner.

* Ved punkttetthet mindre enn 3 punkt pr. kvadratmeter skal det brukes innmålte punkt som xref:imgKontrollflateUnder[xrefstyle=short]. Ved punkttetthet mellom 3 og 70 punkt pr. kvadratmeter skal det være innmålte punkt som xref:imgKontrollflateOver[xrefstyle=short]. Ved punkttetthet over 70 punkt pr. kvadratmeter skal det være innmålte punkt som xref:imgKontrollflateOver70[xrefstyle=short].
...
4+^|Antall lasermålinger innenfor kontrollflaten ved ulike punkttettheter
h| h| h|
^h| 2 punkt/m^2^
^h| 5 punkt/m^2^
^h| 10 punkt/m^2^
...
^|4x4 ^|16 ^|25 ^|32|||  
^|2x2 ^|4 ^|13 ^| ^|20 ^|40| 
^|1,5x1,5 ^|2,25 ^|13|| ^|23 ^|180 
^|0,5x0,5 ^|0,25 ^|5||| ^|20
|===

.Kontrollflate - punkttetthet < 3 punkt/m^2^
...
.Kontrollflate - punkttetthet > 70 punkt/m^2^
...
* Antall kontrollflater skal ikke være under et minimum på 3 flater pr. sammenhengende skanneblokk. Oppdragstaker kan slå sammen mindre områder til sammenhengende skanneblokk for å redusere behov for flater. Se <<#imgSammenslåingBlokk>>. 
...
* Ved terrenglinjer skal det utføres minimum en måling pr. meter, total lengde bør være minimum 10m. Knekkpunkter i terrenget skal måles.
...
** Homogenitetsplottet skal dokumenteres med kartbladvise georefererte TIF-filer med oppløsning 1m, med tilhørende dZ-verdi pr. piksel i meter med 2 desimaler.
====
...
Gjeldende versjon av https://dokument.geonorge.no/produktspesifikasjoner/punktsky/1.0.3/index.html[Produktspesifikasjon Punktsky] detaljerer hvilke klasser som tillates. 
